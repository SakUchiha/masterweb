...
class SyntaxChecker {
  ...
  checkUnclosedTags(html) {
    ...
    while ((match = tagRegex.exec(html)) !== null) {
      ...
      const isSelfClosing =
        match[0].endsWith("/>") || this.isSelfClosingTag(tagName);
  ...
  checkTagNesting(html) {
    ...
    const nestingIssues = [
      {
        pattern: /<p[^>]*>[\s\S]*?<(div|h[1-6])[\s\S]*?<\/p>/gi,
        message: "Block elements cannot be nested inside <p> tags",
      },
      {
        pattern:
          /<(div|span)[^>]*>[\s\S]*?<(div|span)[^>]*>[\s\S]*?<\/(div|span)>[\s\S]*?<\/(div|span)>/gi,
        message: "Check for proper tag nesting",
      },
    ];
  ...
  checkUndefinedVariables(js) {
    ...
    const undefinedPatterns = [
      {
        pattern: /\bundefinedVariable\b/g,
        message: 'Variable "undefinedVariable" is not defined',
      },
      {
        pattern: /\bmyUndefinedFunction\b/g,
        message: 'Function "myUndefinedFunction" is not defined',
      },
    ];
  ...
  checkConsoleLogs(js) {
    if (js.includes("console.log")) {
      this.warnings.push({
        ...
        line: 1,
        suggestion:
          "Remember to remove console.log statements in production code",
      });
    }
  }
  ...
  getSuggestions(errorType, language) {
    ...
    return (
      suggestions[language]?.[errorType] || "Review your code for syntax issues"
    );
  }
}
...
// Also expose as a browser global for pages that include the script directly
if (typeof window !== "undefined") {
  try {
    window.SyntaxChecker = SyntaxChecker;
  } catch (e) {
    // ignore if assignment fails for unusual environments
  }
}
